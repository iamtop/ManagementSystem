package com.infotop.management.branch.web;


import com.infotop.management.branch.service.BranchService;
...
/**
 * BranchAction
 * $Id: BranchAction.java,v 0.0.1 2015-03-23 08:06:25  $
 */
@Controller
...
public class BranchController extends BasicController {
	...
	public DataGrid findList(
		...
		try {
			...
			if (user != null) {
				...
				dataGrid = branchService.dataGrid(searchParams, pageNumber,
						rows, sortType, order);
			} else {
				logger.log(this.getClass(),Logger.ERROR_INT,"登陆帐号无效!","",null);
			}
	 ...
	 */
	 ...
	 @ResponseBody
	 public Message create(@Valid Branch branch, RedirectAttributes redirectAttributes) {
		try {
			ShiroUser su = super.getLoginUser();
			User user = accountService.findUserByLoginName(su.getLoginName());
			if (user != null) {
	 ...
	 */
	 @RequestMapping(value = "update/{id}", method = RequestMethod.GET)
	    public String updateForm(@PathVariable("id") Long id, Model model) {
			...
			User user = accountService.findUserByLoginName(su.getLoginName());
			if (user != null) {
				Branch entity = branchService.get(id); 
		        model.addAttribute("branch", entity);
	 ...
	 */
	 ...
	 @ResponseBody
	    public Message update(@Valid @ModelAttribute("preloadBranch") Branch branch,
	 ...
	 @RequestMapping(value = "view/{id}", method = RequestMethod.GET)
	    public String view(@PathVariable("id") Long id, Model model) {
			...
			if (user != null) {
		 	...
		 	return "branch/branchView";
	    }
	 
	 
	   @ModelAttribute("preloadBranch")
	    public Branch getBranch(@RequestParam(value = "id", required = false) Long id) {
	        if (id != null) {
